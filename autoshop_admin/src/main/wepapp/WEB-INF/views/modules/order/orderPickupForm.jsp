...
<head>
	...
	<script type="text/javascript">
        ...
        debugger;
            var obj = document.getElementsByName("altake_num"); 
			for(var i = 0; i < obj.length; i++)
			 { 
				if((obj[i].value | 0) != obj[i].value || obj[i].value<1 || obj[i].value>999){
					toastr.warning("提货数量只能输入1~999的整数");
					return false;
				}
			 }
			$("#orderPickupForm").submit();
		}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/order/orderApply/pickupForm">提货【${orderApply.order_id}】</a></li>
	</ul><br/>
	<ul class="nav nav-tabs">
		<li class="active"><a href="#main" data-toggle="tab">订单信息</a></li>
		<li><a href="#sub" data-toggle="tab"><span>商品信息</span></a></li>
	</ul><br/>
	<form:form id="orderPickupForm" modelAttribute="orderApply" action="${ctx}/order/orderApply/pickup" method="post" class="form-horizontal">
		...
		<div class="tab-pane fade" id="sub">
				 <table class="table table-striped table-bordered table-condensed">
					...
					<c:forEach items="${orderBoxList}" var="orderBox">
						<tr>
						<td><input type='text' readonly='readonly'  name='product_id' value='${orderBox.product_id}'></td>
						<td><input onmouseover='this.title=this.value' style='white-space:nowrap;overflow:hidden;text-overflow:ellipsis;' type='text' readonly='readonly' name='product_name' value='${orderBox.product_name}'></td>
						<td><input type='text' readonly='readonly' name='box_id' value='${orderBox.box_id}'></td>
						<td><input type='text' readonly='readonly' name='take_num' value='${orderBox.take_num}'></td>
						<td><input type='text' style='width:60px' name='altake_num' value='0'></td>
						</tr>
					</c:forEach>
					</tbody>
				 </table>
		...
		<div class="form-actions">
			<input id="btnSubmit" class="btn btn-primary" type="button" value="提货" onclick="validCheckForm()"/>
