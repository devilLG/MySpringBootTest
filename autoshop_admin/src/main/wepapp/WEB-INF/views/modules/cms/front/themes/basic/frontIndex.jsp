...
<head>
	...
	<meta name="description" content="zhilai ${site.description}" />
	<meta name="keywords" content="zhilai ${site.keywords}" />
</head>
<body>
    ...
    <div class="row">
      <div class="span4">
		...
		<ul><c:forEach items="${fnc:getArticleList(site.logid, 2, 8, '')}" var="article">
      ...
      <div class="span4">
		...
		<ul><c:forEach items="${fnc:getArticleList(site.logid, 6, 8, '')}" var="article">
			<li><span class="pull-right"><fmt:formatDate value="${article.updateDate}" pattern="yyyy.MM.dd"/></span><a href="${article.url}" style="color:${article.color}">${fns:abbr(article.title,28)}</a></li>
		</c:forEach></ul>
      </div>
      <div class="span4">
        <h4><small><a href="${ctx}/list-10${urlSuffix}" class="pull-right">更多&gt;&gt;</a></small>政策法规</h4>
		<ul><c:forEach items="${fnc:getArticleList(site.logid, 10, 8, '')}" var="article">
			<li><span class="pull-right"><fmt:formatDate value="${article.updateDate}" pattern="yyyy.MM.dd"/></span><a href="${article.url}" style="color:${article.color}">${fns:abbr(article.title,28)}</a></li>
		</c:forEach></ul>
      </div>
    </div>
