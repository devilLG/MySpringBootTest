...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			// 用户-菜单
			var zNodes=[
					<c:forEach items="${menuList}" var="menu">{id:"${menu.logid}", pId:"${not empty menu.parent.logid?menu.parent.logid:0}", name:"${not empty menu.parent.logid?menu.name:'权限列表'}"},
		            </c:forEach>];
			// 初始化树结构
			...
			// 用户-机构
			var zNodes2=[
					<c:forEach items="${officeList}" var="office">{id:"${office.logid}", pId:"${not empty office.parent?office.parent.logid:0}", name:"${office.name}"},
		            </c:forEach>];
			// 初始化树结构
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/sys/role/form?logid=${role.logid}">角色<shiro:hasPermission name="sys:role:edit">${not empty role.logid?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:role:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="role" action="${ctx}/sys/role/save" method="post" class="form-horizontal">
		...
		<%-- <div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="office" name="office.logid" value="${role.office.logid}" labelName="office.name" labelValue="${role.office.name}"
