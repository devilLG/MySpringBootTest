...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/sys/menu/form?logid=${menu.logid}&parent.logid=${menu.parent.logid}">菜单<shiro:hasPermission name="sys:menu:edit">${not empty menu.logid?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:menu:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="menu" name="parent.logid" value="${menu.parent.logid}" labelName="parent.name" labelValue="${menu.parent.name}"
					title="菜单" url="/sys/menu/treeData" extId="${menu.logid}" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span class="help-inline">链接地址打开的目标窗口，默认：mainFrame, 其它_blank、_self、_parent、_top</span>
			</div>
		</div>
		<div class="control-group">
