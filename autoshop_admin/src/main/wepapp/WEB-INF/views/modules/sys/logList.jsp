...
<!-- 	</ul> -->
	<form:form id="searchForm" action="${ctx}/sys/log/" method="post" class="breadcrumb form-search">
		...
		<div>
			...
			<label>用户ID：</label><input id="createBy.logid" name="createBy.logid" type="text" maxlength="50" class="input-mini" value="${log.createBy.logid}"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="log">
			<tr>
				<td>${fns:abbr(log.title,20)}</td>
				<td>${fns:abbr(log.createBy.logid,20)}</td>
				<td>${log.createBy.name}</td>
				<td>${log.createBy.company.name}</td>
				<td>${log.createBy.office.name}</td>
				<td><strong>${fns:abbr(log.requestUri,20)}</strong></td>
				<td>${fns:abbr(log.exception,20)}</td>
				<td>${log.method}</td>
				<td>${log.remoteAddr}</td>
				<td><fmt:formatDate value="${log.createDate}" type="both"/></td>
				<td>
    				<a href="${ctx}/sys/log/detail?logid=${log.logid}">详情</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
