...
<body>
	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.logid}}" pId="{{pid}}">
			<td><a href="${ctx}/sys/area/form?logid={{row.logid}}">{{row.name}}</a></td>
			<td>{{row.code}}</td>
			<td>{{dict.type}}</td>
			<td>{{row.remarks}}</td>
			<shiro:hasPermission name="sys:area:edit"><td>
				<a href="${ctx}/sys/area/form?logid={{row.logid}}">修改</a>
				<a href="${ctx}/sys/area/delete?logid={{row.logid}}" onclick="return confirmx('要删除该区域及所有子区域项吗？', this.href)">删除</a>
				<a href="${ctx}/sys/area/form?parent.logid={{row.logid}}">添加下级区域</a> 
