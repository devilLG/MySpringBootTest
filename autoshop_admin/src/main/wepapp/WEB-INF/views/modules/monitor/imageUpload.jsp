...
<html>
	...
	<script>
		...
		var option = {
				language : 'zh',
				uploadUrl : "${ctx}/monitor/monitorDevice/imgUpload",
				uploadAsync : true,
				showCancel :false,
				showColse : false,
				maxFileCount : '6',
		...
		debugger;
		 if(obj.length>0){debugger;
			for(var i = 0 ; i<obj.length;i++){
				...
				var dId = obj[i].substring(obj[i].lastIndexOf("\\")-7,obj[i].lastIndexOf("\\"))
				initialPreview.push(ctx+obj[i])
				initialPreviewConfig.push({
					caption:name,
					url:"${ctx}/monitor/monitorDevice/imgDelete",
					key:i,
					extra:{deviceId:dId,originalName:name}
				});
			}
			debugger;
