...
<script type="text/javascript">
	$("#${id}Button, #${id}Name").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/tag/upgradeselect?url="+encodeURIComponent("${url}"), "${title}",420, 280, {
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"返回":true}, submit:function(v, h, f){
				if (v=="ok"){
				    var myBtn = h.find("iframe")[0].contentWindow.$("input[id='subSave']");
					alert("sucess");
				}else if (v=="返回"){
