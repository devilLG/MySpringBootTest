...
public class CategoryService extends TreeService<CategoryDao, Category> {
	...
	public List<Category> findByUser(boolean isCurrentSite, String module){
		...
		if (list == null){
			...
			for (Category e : list){
				if (e.getParent()!=null && StringUtils.isNotBlank(e.getParent().getLogid())){
					boolean isExistParent = false;
					for (Category e2 : list){
						if (e.getParent().getLogid().equals(e2.getLogid())){
							isExistParent = true;
							break;
						}
					}
					if (!isExistParent){
						parentIdSet.add(e.getParent().getLogid());
					}
				}
			}
...
//				list.addAll(0, dao.find(dc));
		...
		if (isCurrentSite){
			...
			for (Category e : list){
				if (Category.isRoot(e.getLogid()) || (e.getSite()!=null && e.getSite().getLogid() !=null 
						&& e.getSite().getLogid().equals(Site.getCurrentSiteId()))){
	...
	public List<Category> findByParentId(String parentId, String siteId){
		...
		parent.setLogid(parentId);
		entity.setParent(parent);
		Site site = new Site();
		site.setLogid(siteId);
		entity.setSite(site);
...
//		return page;
	...
	public void save(Category category) {
		...
		CmsUtils.removeCache("mainNavList_"+category.getSite().getLogid());
	}
