...
import com.zhilai.master.common.security.Digests;
import com.zhilai.master.common.security.shiro.session.SessionDAO;
import com.zhilai.master.common.service.BaseService;
import com.zhilai.master.common.service.ServiceException;
import com.zhilai.master.common.utils.CacheUtils;
import com.zhilai.master.common.utils.Encodes;
import com.zhilai.master.common.utils.StringUtils;
import com.zhilai.master.common.web.Servlets;
import com.zhilai.master.modules.sys.dao.MenuDao;
import com.zhilai.master.modules.sys.dao.RoleDao;
import com.zhilai.master.modules.sys.dao.UserDao;
import com.zhilai.master.modules.sys.entity.Menu;
...
import com.zhilai.master.modules.sys.security.SystemAuthorizingRealm;
import com.zhilai.master.modules.sys.utils.LogUtils;
 ...
 * 
 * @author zhilai //import com.test.dao.TuserDao;@Autowired private TuserDao
 *         tuserDao;
 * @version 2013-12-05
 */
@Service
...
//				dataScopeFilter(user.getCurrentUser(), "o", "a"));
		// 设置分页参数
		user.setPage(page);
		User userP = UserUtils.getUser();
		if((user.getCorpId() == null || "".equals(user.getCorpId())) && !userP.getCorpId().equals(GParameter.corpId_zhilai)){
			user.setCorpId(userP.getCorpId());
		}
		// 执行分页查询
...
//				dataScopeFilter(user.getCurrentUser(), "o", "a"));
	...
	public void saveUser(User user) {
		if (StringUtils.isBlank(user.getLogid())) {
			...
			// 清除原用户机构用户缓存
			User oldUser = userDao.get(user.getLogid());
			if (oldUser.getOffice() != null
					&& oldUser.getOffice().getLogid() != null) {
				CacheUtils.remove(UserUtils.USER_CACHE,
						UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_
								+ oldUser.getOffice().getLogid());
			}
			// 更新用户数据
			user.preUpdate();
			userDao.update(user);
		}
		if (StringUtils.isNotBlank(user.getLogid())) {
			// 更新用户与角色关联
	...
	public Role getRole(String id) {
		...
		if (r.getOffice() == null) {
			for (Role rl : UserUtils.getRoleList()) {
				if (id.equalsIgnoreCase(rl.getLogid())) {
					r.setOffice(rl.getOffice());
					break;
	...
	public void saveRole(Role role) {
		if (StringUtils.isBlank(role.getLogid())) {
			role.preInsert();
			roleDao.insert(role);
			// 同步到Activiti
	...
	public Boolean outUserInRole(Role role, User user) {
		...
		for (Role e : roles) {
			if (e.getLogid().equals(role.getLogid())) {
				roles.remove(e);
				saveUser(user);
				return true;
			}
	...
	public User assignUserToRole(Role role, User user) {
		...
		if (roleIds.contains(role.getLogid())) {
			return null;
		}
	...
	public void saveMenu(Menu menu) {
		...
		// 获取父节点实体
		menu.setParent(this.getMenu(menu.getParent().getLogid()));

		// 获取修改前的parentIds，用于更新子节点的parentIds
		String oldParentIds = null==menu.getParentIds()?"":menu.getParentIds();

		// 设置新的父节点串
		menu.setParentIds(menu.getParent().getParentIds()
				+ menu.getParent().getLogid() + ",");

		// 保存或更新实体
		if (StringUtils.isBlank(menu.getLogid())) {
			menu.preInsert();
			menuDao.insert(menu);
		} else {
			menu.preUpdate();
	...
	private void saveActivitiGroup(Role role) {
		...
		// 创建用户与用户组关系
		List<User> userList = findUser(new User(new Role(role.getLogid())));
		for (User e : userList) {
