...
import com.zhilai.master.modules.gen.dao.GenDataBaseDictDao;
...
public class GenTableService extends BaseService {
	...
	public GenTable get(String id) {
		...
		genTableColumn.setGenTable(new GenTable(genTable.getLogid()));
	...
	public GenTable getTableFormDb(GenTable genTable){
		...
		if (StringUtils.isNotBlank(genTable.getName())){
			...
			if (list.size() > 0){
				...
				// 如果是新增，初始化表属性
				if (StringUtils.isBlank(genTable.getLogid())){
					genTable = list.get(0);
					// 设置字段说明
	...
	public void save(GenTable genTable) {
		...
		for (GenTableColumn column : genTable.getColumnList()){
			...
			if (StringUtils.isBlank(column.getLogid())){
				column.preInsert();
	...
	public void delete(GenTable genTable) {
		...
		genTableColumnDao.deleteByGenTableId(genTable.getLogid());
	}
