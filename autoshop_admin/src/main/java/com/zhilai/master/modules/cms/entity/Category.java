 ...
 */
package com.zhilai.master.modules.cms.entity;
...
//	}
	...
	public static void sortList(List<Category> list, List<Category> sourcelist, String parentId){
		for (int i=0; i<sourcelist.size(); i++){
			...
			if (e.getParent()!=null && e.getParent().getLogid()!=null
					&& e.getParent().getLogid().equals(parentId)){
				list.add(e);
				// 判断是否还有子节点, 有则继续获取子节点
				for (int j=0; j<sourcelist.size(); j++){
					Category child = sourcelist.get(j);
					if (child.getParent()!=null && child.getParent().getLogid()!=null
							&& child.getParent().getLogid().equals(e.getLogid())){
						sortList(list, sourcelist, e.getLogid());
						break;
					}
				}
			}
	...
	public String getIds() {
		return (this.getParentIds() !=null ? this.getParentIds().replaceAll(",", " ") : "") 
				+ (this.getLogid() != null ? this.getLogid() : "");
	}

	public boolean isRoot(){
		return isRoot(this.logid);
	}
