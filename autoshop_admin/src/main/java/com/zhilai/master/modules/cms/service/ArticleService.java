...
import com.zhilai.master.modules.cms.dao.ArticleDao;
import com.zhilai.master.modules.cms.dao.ArticleDataDao;
import com.zhilai.master.modules.cms.dao.CategoryDao;
...
//		dc.createAlias("category.site", "category.site");
		if (article.getCategory()!=null && StringUtils.isNotBlank(article.getCategory().getLogid()) && !Category.isRoot(article.getCategory().getLogid())){
			Category category = categoryDao.get(article.getCategory().getLogid());
			if (category==null){
				category = new Category();
			}
			category.setParentIds(category.getLogid());
...
//		return dao.find(page, dc);
	...
	public void save(Article article) {
		...
		if (StringUtils.isBlank(article.getLogid())){
			article.preInsert();
			articleData = article.getArticleData();
			articleData.setLogid(article.getLogid());
			articleData.setLogid(article.getLogid());
			dao.insert(article);
...
//		dao.updateDelFlag(id, isRe!=null&&isRe?Article.DEL_FLAG_NORMAL:Article.DEL_FLAG_DELETE);
	...
	public List<Object[]> findByIds(String ids) {
		...
		for(int i=0;(idss.length-i)>0;i++){
			e = dao.get(idss[i]);
			list.add(new Object[]{e.getCategory().getLogid(),e.getLogid(),StringUtils.abbr(e.getTitle(),50)});
		}
		return list;
	}
